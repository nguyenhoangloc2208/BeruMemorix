...
async function testMemoryOptimizationAdvisor() {
  ...
  await memoryStorage.store("Fuzzy search implementation using Levenshtein distance", {
    title: "Fuzzy Search Guide",
    category: "development", 
    tags: ["search", "algorithm", "fuzzy"]
  });
  
  await memoryStorage.store("Fuzzy string matching with edit distance calculations", {
  ...
  console.log(`   Duplicate risk: ${(healthReport.duplicateRisk * 100).toFixed(1)}%`);
  console.log(`   Consolidation opportunities: ${healthReport.consolidationOpportunities}`);
  ...
  if (healthReport.recommendedActions.length > 0) {
    ...
    healthReport.recommendedActions.slice(0, 3).forEach((rec, index) => {
      console.log(`   ${index + 1}. [${rec.priority.toUpperCase()}] ${rec.type}: ${rec.reason}`);
      console.log(`      Target: ${rec.targetMemoryId}, Confidence: ${(rec.confidence * 100).toFixed(0)}%`);
    });
  }
  ...
  console.log(`   Confidence: ${(updateSuggestion.confidence * 100).toFixed(0)}%`);
  
  if (updateSuggestion.targetMemory) {
    console.log(`   Target memory: ${updateSuggestion.targetMemory.metadata.title || 'Untitled'}`);
  }
...
async function testMemoryGuidanceSystem() {
  ...
  if (healthAssessment.recommendations.length > 0) {
    console.log(`   Recommendations: ${healthAssessment.recommendations.join(', ')}`);
  }
...
async function testUpdateMemoryOperations() {
  ...
  if (memories.length > 0) {
    ...
    console.log(`🎯 Testing with memory: ${testMemory.metadata.title || 'Untitled'}`);
    ...
    const similarMemories = memories.filter(mem => 
      mem.id !== testMemory.id && 
      mem.metadata.category === testMemory.metadata.category
    );
    
    if (similarMemories.length > 0) {
      console.log(`🔄 Found ${similarMemories.length} potential merge candidates`);
    ...
    const consolidationCandidates = memories.filter(mem =>
      mem.metadata.category === "development"
    );
    
    console.log(`📚 Consolidation candidates: ${consolidationCandidates.length} development memories`);
    
    if (consolidationCandidates.length >= 3) {
      console.log(`✨ Recommendation: Use update_memory 'consolidate' for development category`);
    }
  }
...
async function demonstrateMemoryOptimizationWorkflow() {
  ...
  const context = {
    ...
    currentMemoryCount: await memoryStorage.getStats().then(s => s.totalMemories)
  };
  
  console.log(`📝 New content to store: "${newContent}"`);
  
  // Step 1: Analyze content for guidance
  const guidance = MemoryGuidanceSystem.analyzeContentForGuidance(newContent, context);
  ...
  if (guidance.shouldUseUpdate) {
    ...
    if (smartSuggestion.targetMemory) {
      console.log(`   Target memory: ${smartSuggestion.targetMemory.metadata.title}`);
      console.log(`   ✅ RECOMMENDATION: Use update_memory with operation '${smartSuggestion.updateType}'`);
    }
  } else {
    console.log(`   ✅ RECOMMENDATION: Safe to use store_memory for new content`);
  }
...
async function runAllTests() {
  ...
  try {
    ...
    console.log(`✅ Memory Optimization Advisor: Working - Smart recommendations & analysis`);
    console.log(`✅ Memory Guidance System: Working - Intelligent AI guidance`);
    console.log(`✅ Enhanced Update Operations: Working - Multiple operation types`);
    console.log(`✅ Optimization Workflow: Working - Complete decision support`);
    console.log(`\n🚀 AI Memory Optimization Complete!`);
    console.log(`📈 AI now has intelligent guidance for using update_memory efficiently`);
    console.log(`🎯 Memory usage will be automatically optimized through smart recommendations`);
